...
class RegisterBloc extends Bloc<RegisterEvent, RegisterState> {
  ...
  void onCheckEmailEvent(
    ...
    emit(await _checkEmailUseCase(event.data["email"]).then((value) =>
        value.fold(
            (l) => RegisterErrorState(message: l.errorMessage),
            (r) =>
                CheckEmailLoadedState(secureKey: r.token, data: event.data))));
  }

  void onResendCodeEvent(
      ResendCodeEvent event, Emitter<RegisterState> emit) async {
    emit(RegisterLoadingState());
    emit(await _resendCodeUseCase(event.email).then((value) => value.fold(
        (l) => RegisterErrorState(message: l.errorMessage),
        (r) => ResendCodeLoadedState(secureKey: r.token))));
  }
